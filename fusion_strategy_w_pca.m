function [gen, ave1, ave2, ave3, ave4] = fusion_strategy_w_pca(features_a, features_b,...
    source_a, source_b, img1, img2, img_1, img_2, unit)
...
for i=2:m-1
    for j=2:n-1
        ...
        % weight average
        weight_ave_temp1(((i-2)*unit+1):((i-1)*unit),((j-2)*unit+1):((j-1)*unit))...
            = A1/(A1+A2+A3+A4);
        weight_ave_temp2(((i-2)*unit+1):((i-1)*unit),((j-2)*unit+1):((j-1)*unit))...
            = A2/(A1+A2+A3+A4);
        weight_ave_temp3(((i-2)*unit+1):((i-1)*unit),((j-2)*unit+1):((j-1)*unit))...
            = A3/(A1+A2+A3+A4);
        weight_ave_temp4(((i-2)*unit+1):((i-1)*unit),((j-2)*unit+1):((j-1)*unit))...
            = A4/(A1+A2+A3+A4);
...
gen = source_a.*weight_ave_temp1 + source_b.*weight_ave_temp2 + img_1.*weight_ave_temp3...
    + img_2.*weight_ave_temp4;
